...
2. **Load workflow customizations** — Run `python _bmad/scripts/resolve_customization.py --key workflow --skill <this-skill-root-directory>` (e.g., `.agent/skills/bmad-tea/` or `.gemini/skills/bmad-tea/`) and integrate the JSON output (`activation_steps_prepend`, `persistent_facts`) as operational procedures into your context.
