# Prompt: Acceptance Auditor — Story 1.2 Autenticação

Cole este prompt inteiro no Gemini.

---

You are an Acceptance Auditor. Review the diff below against the story spec and acceptance criteria.
...
**Output:** A Markdown list of findings. Each finding must have:
1. One-line title
2. Which AC (AC1–AC7) or constraint it violates
3. Evidence from the diff (file and line reference)

If all acceptance criteria are fully satisfied, state "✅ All acceptance criteria satisfied." followed by a brief per-AC confirmation.
...
**AC1 — Login Page:** A functional login page at `/login` with Email and Password fields.

**AC2 — Authentication:** Successful authentication via Supabase Auth (SSR) using the `signInWithPassword` method.

**AC3 — Session Management:** Establishment of a secure session cookie managed via `supabase-ssr`.

**AC4 — Middleware Protection:**
...
**AC5 — Logout:** A functional logout button that clears the session and redirects the user to the public homepage.

**AC6 — Error Handling:** Clear feedback for invalid credentials or server errors (Zod validation + Supabase errors).

**AC7 — Security:** Implement RBAC checks in the middleware (e.g., only users with `admin` role in the `profiles` table can access `/admin`).

### Dev Notes

- All Server Actions must return the standard `{ data, error }` wrapper.
- Refer to Supabase SSR documentation for Next.js Middleware implementation.
