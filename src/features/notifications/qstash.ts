...
export type AdminWhatsAppPayload = {
...
export async function enqueueAdminWhatsAppNotification(
    payload: AdminWhatsAppPayload
): Promise<{ queued: boolean; error?: string }> {
    if (!process.env.QSTASH_TOKEN) {
        return { queued: false, error: "QSTASH_TOKEN not configured" };
    }
    ...
    try {
        await qstashClient.publishJSON({
            url: new URL("/api/webhooks/notifications/admin-whatsapp", baseUrl).toString(),
            body: payload,
        });
        return { queued: true };
    } catch (err: unknown) {
