...
2. **Load workflow customizations** — Run `python _bmad/scripts/resolve_customization.py --key workflow --skill <this-skill-root-directory>` (e.g., `.agent/skills/bmad-tea/` for OpenCode, or `.claude/skills/bmad-tea/` for Claude Code) and integrate the JSON output (`activation_steps_prepend`, `persistent_facts`) as operational procedures into your context.
