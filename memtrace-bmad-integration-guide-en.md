...
**CRITICAL — Dual-section approach for agent persona skills:** Skills of `type: agent` (personas like Murat, John, Winston, Amelia) have TWO sections in their `.toml` file. The framework resolves `--key agent` automatically; the agent additionally loads `--key workflow` via a SKILL.md activation step. This keeps methodology rules (`persistent_facts`) separate from execution steps (`activation_steps_prepend`).

- **Agent persona skills** (`bmad-agent-dev`, `bmad-agent-pm`, `bmad-agent-architect`, `bmad-agent-tech-writer`, `bmad-agent-analyst`, `bmad-agent-ux-designer`, `bmad-tea`) → use `[agent]` for `persistent_facts` + `[workflow]` for `activation_steps_prepend` (the SKILL.md On Activation step tells the agent to also load `--key workflow`)
- **Workflow skills** (`bmad-dev-story`, `bmad-code-review`, `bmad-create-story`, `bmad-sprint-planning`, `bmad-retrospective`, `bmad-correct-course`, `bmad-quick-dev`, `bmad-create-epics-and-stories`, `bmad-create-architecture`, `bmad-check-implementation-readiness`, `bmad-document-project`, `bmad-generate-project-context`, `bmad-qa-generate-e2e-tests`) → use `[workflow]` only (these are invoked as workflows and resolve `--key workflow`)

**File format — Agent persona skills (`[agent]` + `[workflow]`):**
```toml
# {install_scope_path}/bmad-{agent-name}.toml
# Methodology rules — resolved by framework via --key agent
[agent]
persistent_facts = [
    "BEFORE editing any function, run get_impact...",
]

# Execution steps — loaded by agent via SKILL.md step resolving --key workflow
[workflow]
...
> Agent-type skills use the dual-section approach: `[agent]` for methodology rules, `[workflow]` for execution steps.
> Workflow-type skills use `[workflow]` only. The "Sections" column shows which sections the `.toml` file contains.

| File | Sections | Agent | Runs automatically on start |
|------|----------|-------|---------------------------|
| `bmad-dev-story.toml` | `[workflow]` | Amelia — Dev (`/bmad-agent-dev`) | `get_codebase_briefing` + `find_code` on the feature |
| `bmad-agent-dev.toml` | `[agent]` + `[workflow]` | Amelia — Dev (`/bmad-agent-dev`) | `get_codebase_briefing` + `find_code` on the feature + `get_changes_since` |
...
| `bmad-tea.toml` | `[agent]` + `[workflow]` | Murat — QA (`/bmad-tea`) | `list_processes` + `find_symbol` on exported functions |
...
| `bmad-agent-pm.toml` | `[agent]` + `[workflow]` | John — PM (`/bmad-agent-pm`) | `get_codebase_briefing` + `find_most_complex_functions` |
| `bmad-agent-architect.toml` | `[agent]` + `[workflow]` | Winston — Architect (`/bmad-agent-architect`) | `list_communities` + `get_api_topology` |
...
**For each file listed above:** create the corresponding `.toml` file with `activation_steps_prepend` + `persistent_facts`. Use the Sections column to determine whether the file uses `[workflow]` only or both `[agent]` + `[workflow]`.

> **SKILL.md activation step for agent-type skills:** For each agent persona `.toml` file (dual-section), the agent's `SKILL.md` must include an "On Activation" step that resolves the `[workflow]` section at runtime. Add the following to the agent's SKILL.md:
> ```
> 2. **Load workflow customizations** — Run `python _bmad/scripts/resolve_customization.py --key workflow --skill <this-skill-root-directory>` (e.g., `.agent/skills/{skill-name}/` for OpenCode) and integrate the JSON output (`activation_steps_prepend`) as operational procedures into your context.
> ```

---

#### Level 2: `persistent_facts` — Methodology rules the agent considers THROUGHOUT the session
...
> **Dual-section placement:** For agent persona skills, `persistent_facts` go in `[agent]` (framework-resolved methodology rules) and `activation_steps_prepend` go in `[workflow]` (agent-loaded execution steps). For workflow skills, both stay in `[workflow]`.
