...
- 2026-04-27: Série de 7 iterações de debugging de E2E/CI por `claude-opus-4-7`. Problemas resolvidos: (1) secrets invertidos (SUPABASE_URL ↔ DATABASE_URL); (2) drizzle-kit silenciava erros de migração → criado `scripts/migrate.mjs` com API programática que aplica todos os .sql em ordem lexical com tracking próprio em `_manual_migrations`; (3) URL com trailing slash no SUPABASE_URL; (4) strict mode violation no Playwright (`text=Reivindicar Perfil` resolvia para 2 elementos) → trocado para `getByRole('button')`; (5) `result.rowCount` sempre `undefined` no postgres-js → `claimArtistProfileAction` sempre retornava ALREADY_CLAIMED — corrigido com `.returning({ id })`; (6) execução paralela de workers compartilhando seed e unique constraint `artists.profile_id` → `test.describe.configure({ mode: 'serial' })` + reset de DB em `beforeEach`. 18/18 testes passando. PR #12 merged. Status → done.
